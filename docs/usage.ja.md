...
インストール後の実際の使い方を、新規プロジェクト・後入れ（運用中プロジェクトへの途中導入）の両方について説明する。インストール手順そのものは [README の Quickstart](../README.ja.md#クイックスタート)、仕組みの図解は [architecture.ja.md](architecture.ja.md) を参照。
