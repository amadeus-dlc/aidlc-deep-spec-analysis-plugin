...
3. 質問に答える:
   ...
   - **B. 改訂案を採用** — 提示された要件改訂案を採用する（**適用はあなたの作業**。プラグインが `requirements.md` を書き換えることはない）
   - **X. その他** — 自由回答
4. `deep-spec-analysis-report.md` を読む。義務×バックエンドのカバレッジ表と、そのまま適用できる改訂案が並ぶ。
...
- **`requirements.md` が書き換わってしまう不安** — プラグインは `requirements.md` を編集しない。Bを選んだ場合も適用は人間の作業として残る。
