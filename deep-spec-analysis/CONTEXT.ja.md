...
**属性カタログ項目**:
属性宣言と、その所有元の実体を組にしたもの。完全な属性パスは所有元と属性名から導出される。カタログの絞り込みでも、この対応を保つ。
...
**問いの判定項目**:
問いを識別するラベルと判定の組。判定を選び出す操作でも、どの問いに対する判定かを保つ。

**トレース状態**:
一つの時点における、属性パスと観測値の対応。属性パスの列挙順が異なっても、同じ属性パスに等価な値が対応していれば同じ状態とする。

**トレース**:
観測された状態をステップ順に並べたもの。ステップの順序が変われば別のトレースになる。

**トレース状態項目**:
一つのトレース状態で観測された、属性パスと値の組。パスを項目に含めることで、別の属性の値を区別する。
...
**兄弟ユニット索引項目**:
一つのユニット名と、そのユニットの実体宣言集合の組。実体宣言がないユニットも、一つの索引項目として保持する。
...
**設置台帳**:
検証ツールの設置状態を確認するために列挙された、少なくとも一項目を持つ台帳。

**台帳項目**:
設置確認に挙げられた成果物一つと、その成果物が欠けている場合の診断の重大度。項目を選択した結果は空になり得る。
