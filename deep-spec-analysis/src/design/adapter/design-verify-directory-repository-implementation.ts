...
export class DesignVerifyDirectoryRepositoryImplementation implements DesignVerifyDirectoryRepository {
  ...
  findByDirectory(directory: ArtifactPath): Result<DesignVerifyDirectory, RepositoryError> {
    ...
    if (!crossText.ok)
      return ok(
        DesignVerifyDirectory.unreadableCrossCheck(directory, reports.value, crossCheckFailure(crossText.error)),
      );
    const crossCheck = this.#parseReport(directory, CROSS_CHECK_BASENAME, crossText.value);
    if (!crossCheck.ok)
      return ok(
        DesignVerifyDirectory.unreadableCrossCheck(directory, reports.value, crossCheckFailure(crossCheck.error)),
      );
  ...
  #parseReport(directory: ArtifactPath, fileName: string, text: string): Result<DesignReport, RepositoryError> {
    const path = join(directory.asString(), fileName);
    let raw: Json;
    try {
