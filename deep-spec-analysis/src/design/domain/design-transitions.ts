...
export class DesignTransitions extends FirstClassCollectionBase<DesignTransition, DesignTransitions> {
  ...
  protected override rebuild(values: readonly DesignTransition[]): DesignTransitions {
  ...
  override map(transform: (element: DesignTransition) => DesignTransition): DesignTransitions {
    return this.mapTo(transform, DesignTransitions.of);
  }

  override combine(other: DesignTransitions): DesignTransitions {
  ...
  override *[Symbol.iterator](): Iterator<DesignTransition> {
    yield* this.#values;
  }
