...
export class EffectAssignments {
  ...
  private constructor(effect: Expression) {
    ...
    for (const term of terms) {
      if (term.op !== "eq" || term.args?.length !== 2) throw new IllegalArgumentException({ kind: "effect-not-assignment-conjunction" });
      const [a, b] = term.args ?? [];
      const target = a?.op === "ref" && a.prime === true ? a : b?.op === "ref" && b.prime === true ? b : null;
      if (!target || target.path === undefined) throw new IllegalArgumentException({ kind: "effect-not-assignment-conjunction" });
