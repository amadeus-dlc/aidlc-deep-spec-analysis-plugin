...
type ContractRowParam = { id: ContractIdentifier; provider: ContractParty; consumer: ContractParty; owner: ContractParty; line: LineNumber };
...
export class ContractRow {
  ...
  checkPartiesDeclared(declared: UnitDeclarations, report: ReferenceCheckReport, artifact: ArtifactPath, depArtifact: ArtifactPath): void {
    ...
    if (!this.#provider.isBlank() && !declared.declares(this.#provider.asString())) {
      report.finding(CD_1, FindingKind.referenceBroken(), [`contract:${this.#id.asString()}`, TargetIdentifiers.safe("unit", this.#provider.asString())],
    ...
    if (!this.#consumer.isBlank() && !declared.declares(this.#consumer.asString()) && !this.#consumer.declaresExternal()) {
      report.finding(CD_1, FindingKind.referenceBroken(), [`contract:${this.#id.asString()}`, TargetIdentifiers.safe("unit", this.#consumer.asString())],
    ...
    if (!this.#owner.isBlank() && !declared.declares(this.#owner.asString())) {
      report.finding(CD_1, FindingKind.referenceBroken(), [`contract:${this.#id.asString()}`, TargetIdentifiers.safe("unit", this.#owner.asString())],
