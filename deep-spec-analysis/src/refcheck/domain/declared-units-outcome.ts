...
export class DeclaredUnitsOutcome {
  ...
  private constructor(kind: "absent" | "unrecognized" | "declared", error: string | undefined, units: UnitDeclarations | null) {
    this.#kind = kind;
    this.#error = error;
    this.#units = units;
  }
  ...
  match<T>(handlers: { absent: () => T; unrecognized: (error: string | undefined) => T; declared: (units: UnitDeclarations) => T }): T {
