  ...
  *[Symbol.iterator](): Iterator<RuleDecl> {
  ...
  check(report: ReferenceCheckReport, artifact: ArtifactPath, requirementIdsKnown: RequirementIds | null, entities: DeclaredEntities | null): void {
    ...
    for (const r of this) {
      ...
      if (id === null) continue;
      if (!id.matchesShape()) {
        report.finding(FD_R2, FindingKind.structureInvalid(), [FD_R2.asCheckTarget()], [WitnessRef.at(art, `${r.element().asString()}.id`, id.asString())],
          `rule id "${id.asString()}" does not match BR{group}.{seq}`);
        continue;
      }
      if (seenIds.has(id.asString())) {
        report.finding(FD_R2, FindingKind.structureInvalid(), [id.asString()], [WitnessRef.at(art, `${r.element().asString()}.id`, id.asString())],
