  ...
  check(report: ReferenceCheckReport, artifact: ArtifactPath): void {
    ...
    for (const c of this) {
      if (!c.nameIsPascalCase()) {
        ...
        report.finding(DD_1, FindingKind.structureInvalid(), [TargetIds.safe("component", cName)], [WitnessRef.at(art, `${c.element().asString()}.name`, cName)],
    ...
    for (const { prior, current } of this.duplicateNamePairs()) {
      ...
      report.finding(DD_1, FindingKind.structureInvalid(), [TargetIds.safe("component", cName)],
        [WitnessRef.at(art, `${prior.element().asString()}.name`, cName), WitnessRef.at(art, `${current.element().asString()}.name`, cName)],
    ...
    for (const c of this) {
      for (const r of [...c.dependsOn(), ...c.dependents()]) {
        if (!this.declares(r.component())) {
          report.finding(DD_2, FindingKind.referenceBroken(), [TargetIds.safe("component", r.component().asString())], [WitnessRef.at(art, r.element().asString(), r.component().asString())],
      ...
      for (const e of c.entities()) {
        for (const r of e.references()) {
          if (!this.declares(r.ownedBy())) {
            report.finding(DD_2, FindingKind.referenceBroken(), [TargetIds.safe("component", r.ownedBy().asString())], [WitnessRef.at(art, `${r.element().asString()}.owned_by`, r.ownedBy().asString())],
    ...
    for (const c of this) {
      for (const r of c.selfReferences()) {
        report.finding(DD_3, FindingKind.structureInvalid(), [TargetIds.safe("component", c.name().asString())], [WitnessRef.at(art, r.element().asString(), c.name().asString())],
    ...
    for (const c of this) {
      for (const r of c.dependsOn()) {
        ...
        if (!other.dependents().listsComponent(c.name())) {
          report.finding(DD_4, FindingKind.structureInvalid(), [TargetIds.safe("component", c.name().asString()), TargetIds.safe("component", r.component().asString())],
            [WitnessRef.at(art, r.element().asString(), r.component().asString()), WitnessRef.at(art, `${other.element().asString()}.dependents`, c.name().asString())],
      ...
      for (const r of c.dependents()) {
        ...
        if (!other.dependsOn().listsComponent(c.name())) {
          report.finding(DD_4, FindingKind.structureInvalid(), [TargetIds.safe("component", c.name().asString()), TargetIds.safe("component", r.component().asString())],
            [WitnessRef.at(art, r.element().asString(), r.component().asString()), WitnessRef.at(art, `${other.element().asString()}.depends_on`, c.name().asString())],
    ...
    for (const c of this) {
      for (const e of c.entities()) {
        if (!e.hasIdentifier()) {
          report.finding(DD_5, FindingKind.structureInvalid(), [TargetIds.safe("entity", e.name().asString())], [WitnessRef.at(art, `${e.element().asString()}.identifier`)],
    ...
    for (const conflict of this.ownershipConflicts()) {
      ...
      report.finding(DD_5, FindingKind.structureInvalid(), [TargetIds.safe("entity", name)],
        conflict.owners.map((o) => WitnessRef.at(art, o.entity.element().asString(), o.component.name().asString())),
      ...
      for (const e of c.entities()) {
        for (const r of e.references()) {
          ...
          if (!owner.entities().declaresEntity(r.entity())) {
            report.finding(DD_6, FindingKind.referenceBroken(), [TargetIds.safe("entity", r.entity().asString())], [WitnessRef.at(art, `${r.element().asString()}.entity`, r.entity().asString())],
