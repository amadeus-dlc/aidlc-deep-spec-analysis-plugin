...
export class DeclaredEntities {
  ...
  check(report: ReferenceCheckReport, artifact: ArtifactPath): void {
    ...
    for (const dup of this.entities().duplicatesByName()) {
      report.finding(FD_E1, FindingKind.structureInvalid(), [TargetIds.safe("entity", dup.name().asString())], [WitnessRef.at(art, `${dup.element().asString()}.name`, dup.name().asString())],
        `entity "${dup.name().asString()}" is declared more than once`);
    }
    for (const e of this.entities()) {
      for (const dup of e.attrs().duplicatesByName()) {
        report.finding(FD_E1, FindingKind.structureInvalid(), [TargetIds.safe("attr", `${e.name().asString()}.${dup.name().asString()}`)], [WitnessRef.at(art, `${dup.element().asString()}.name`, dup.name().asString())],
    ...
    for (const e of this.entities()) {
      for (const a of e.attrs()) {
        ...
        if (a.declaresAllowedValuesOnNonEnumerableType()) {
          report.finding(FD_E2, FindingKind.structureInvalid(), [attrId], [WitnessRef.at(art, a.element().asString(), a.typeToken())],
        ...
        if (a.boundsInverted()) {
          report.finding(FD_E3, FindingKind.structureInvalid(), [attrId], [WitnessRef.at(art, a.element().asString(), `min ${a.min()?.asNumber()} > max ${a.max()?.asNumber()}`)],
            `"${label}": min ${a.min()?.asNumber()} exceeds max ${a.max()?.asNumber()}`);
        }
        if (a.defaultBelowMin()) {
          report.finding(FD_E3, FindingKind.structureInvalid(), [attrId], [WitnessRef.at(art, a.element().asString(), a.def()?.render() ?? "")],
        ...
        if (reference !== null && !this.entities().resolvesReference(reference)) {
          report.finding(FD_E6, FindingKind.referenceBroken(), [attrId], [WitnessRef.at(art, a.element().asString(), reference.asString())],
    ...
    for (const r of this.allRels()) {
      for (const endpoint of [r.from(), r.to()]) {
        if (endpoint !== null && !this.entities().containsNamed(endpoint)) {
          report.finding(FD_E4, FindingKind.referenceBroken(), [TargetIds.safe("entity", endpoint.asString())], [WitnessRef.at(art, r.element().asString(), endpoint.asString())],
            `relationship endpoint "${endpoint.asString()}" is not a declared entity`);
        }
      }
      if (r.cardinalityOutsideClosedSet()) {
        report.finding(FD_E5, FindingKind.structureInvalid(), [FD_E5.asCheckTarget()], [WitnessRef.at(art, r.element().asString(), r.cardinality()?.asString() ?? "")],
          `cardinality "${r.cardinality()?.asString()}" is not in the closed set 1:1 | 1:N | N:1 | N:M`);
      }
      if (r.cardinalityWithoutDirection()) {
        report.finding(FD_E5, FindingKind.structureInvalid(), [FD_E5.asCheckTarget()], [WitnessRef.at(art, r.element().asString())],
