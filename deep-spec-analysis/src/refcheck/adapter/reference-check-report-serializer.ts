...
import {
  Finding, Skipped, WitnessReference, Findings, InputAnchors, ReferenceCheckReport,
  type ReferenceCheckReportIdentifier, Skips, WitnessReferences, InputAnchor, ElementPath,
...
export function parseReportDocument(id: ReferenceCheckReportIdentifier, raw: Json): Result<ReferenceCheckReport, { cause: string }> {
  ...
  for (const entry of doc.findings) {
    ...
    for (const rawRef of Array.isArray(witness.refs) ? witness.refs : []) {
      ...
      refs.push(WitnessReference.of({ artifact: parsed.value.artifact.asString(), element: parsed.value.element.asString(),
  ...
  return ok(ReferenceCheckReport.of({
    ...
    checked: TargetIdentifiers.of(checked.value), findings: Findings.of(findings),
