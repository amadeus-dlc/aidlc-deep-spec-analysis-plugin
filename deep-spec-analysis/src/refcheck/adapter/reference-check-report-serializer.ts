import { ArtifactPath, TargetId, TargetIds } from "@deep-spec/kernel-domain";
import { parseFindingsValues } from "@deep-spec/kernel-adapter";
import { type Result, type Json, isObject, combineResults, traverseResult, err, ok } from "@deep-spec/kernel-infrastructure";
import {
  Finding, Skipped, WitnessRef, Findings, InputAnchors, ReferenceCheckReport,
  type ReferenceCheckReportId, Skips, WitnessRefs, InputAnchor, ElementPath,
...
export function parseReportDocument(id: ReferenceCheckReportId, raw: Json): Result<ReferenceCheckReport, { cause: string }> {
  const decoded = parseFindingsValues(raw);
  if (!decoded.ok) return err({ cause: decoded.error });
  const doc = decoded.value;
  if (!doc.backend.equals(id.backendName())) return err({ cause: `document backend "${doc.backend.asString()}" does not match the id backend "${id.backendName().asString()}"` });
  if (doc.inputs === undefined || doc.checked === undefined) return err({ cause: "document lacks inputs/checked/findings/skipped arrays" });
  ...
  for (const entry of doc.findings) {
    ...
    for (const rawRef of Array.isArray(witness.refs) ? witness.refs : []) {
      ...
      refs.push(WitnessRef.of({ artifact: parsed.value.artifact.asString(), element: parsed.value.element.asString(),
  ...
  return ok(ReferenceCheckReport.of({
    ...
    checked: TargetIds.of(checked.value), findings: Findings.of(findings),
    skipped: Skips.of(doc.skipped.map(Skipped.of)), unavailableReason: doc.unavailable?.reason ?? null,
  }));
}
