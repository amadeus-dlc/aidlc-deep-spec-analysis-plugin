...
//（逆引き検証の材料）。キーは RequirementId、値は FunctionalRequirementReferenceClaims、内側は KeyedIndex
...
export class FunctionalRequirementReferenceIndex {
  readonly #ownersByRef: KeyedIndex<RequirementId, FunctionalRequirementReferenceClaims>;

  private constructor(ownersByRef: KeyedIndex<RequirementId, FunctionalRequirementReferenceClaims>) {
  ...
  static of(claims: readonly FunctionalRequirementReferenceClaim[]): FunctionalRequirementReferenceIndex {
    ...
    return new FunctionalRequirementReferenceIndex(KeyedIndex.of([...ownersByRef].map(([ref, owners]) => [RequirementId.of(ref), FunctionalRequirementReferenceClaims.of(owners)] as const)));
  }
