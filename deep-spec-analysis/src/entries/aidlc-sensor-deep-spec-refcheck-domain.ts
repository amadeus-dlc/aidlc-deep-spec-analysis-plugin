...
// on stdout and exit 0. A missing/invalid --output-path, acquisition failure, or report save failure
...
import { CheckDomainComponentsUseCase } from "@deep-spec-analysis/refcheck-usecase";
...
function main(): void {
  ...
  if (outcome.kind === "acquisition-failed") {
    process.stderr.write(
      `deep-spec-refcheck-domain: failed to read ${outcome.error.path}: ${outcome.error.kind}${"cause" in outcome.error ? ` (${outcome.error.cause})` : ""}\n`,
    );
    process.exit(1);
  }
