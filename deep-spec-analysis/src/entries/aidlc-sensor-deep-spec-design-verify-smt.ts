...
import { VerifyDesignSatisfiabilityModuloTheoriesUseCase } from "@deep-spec-analysis/design-usecase";
import {
  ...
  switch (outcome.kind) {
    ...
    case "save-failed":
      process.stderr.write(
        `deep-spec-design-verify-smt: ${outcome.error.path}: ${outcome.error.kind}${"cause" in outcome.error ? ` (${outcome.error.cause})` : ""}\n`,
      );
      process.exit(1);
      break;
    case "verified": {
