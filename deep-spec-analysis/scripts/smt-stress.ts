...
const hogs = Array.from({ length: loadWorkers }, () =>
  spawn("bun", ["-e", "let x=0; const a=[]; while(true){x=(x*31+1)%1e9; a.push({x}); if(a.length>5e4) a.length=0;}"], {
    stdio: "ignore",
  }),
);
...
try {
  for (let i = 0; i < iterations; i++) {
    ...
    try {
      ...
      const smt = readFileSync(
        join(record, "construction", "deep-spec-analysis-functional-verify", "deep-spec-design-verify", "smt.json"),
        "utf-8",
      );
      const h = createHash("sha256").update(smt).digest("hex").slice(0, 12);
      hashes.set(h, (hashes.get(h) ?? 0) + 1);
      const doc = JSON.parse(smt) as {
        findings?: { kind: string; targets: string[]; witness?: { model?: { [k: string]: unknown } } }[];
      };
      const gap = (doc.findings ?? []).find(
        (f) => f.kind === "completeness-gap" && JSON.stringify(f.targets) === JSON.stringify(["SM-1", "TR-3", "TR-4"]),
      );
