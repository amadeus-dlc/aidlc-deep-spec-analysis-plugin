...
## 宣言・比較・取得結果の契約を所有者へ集約する（2026-09-06）

ドメイン上位メソッドの追加精査で見つかった5項目を是正した。

- 状態機械の指定は、構築時に全体サイズ・字句・構文と、実体名・属性名それぞれの契約を確認する。`parse`成功後の照会で初めて名前VOを構築しない。不正な見出しや状態名は明示的な`unrecognized-format`として扱う。
- FD-Sの図と属性の解決は、既存の「明示属性→status/state→allowed valuesを持つ唯一の属性」の順序を維持する。各図の検査に加え、ライフサイクル実体ごとの被覆を確認し、一部の図を追加しただけで他実体の未検査が消えることを防ぐ。属性を解決できない図は被覆として数えない。
- 兄弟ユニットの索引は、型付きのユニット名と実体宣言から構築する。外側だけをコピーしたMapや、呼出元へ返す可変レコードを保持しない。XS一実行の予算はユニット数・実体宣言数それぞれ65,536件とし、索引化の前に確認する。取得失敗は`Result`で運び、XSの未検査を記録する。
- 精緻化写像の取得結果は、既存の`DesignInputAnchors`を受け取り、取得時の証跡を不変に保つ。不在と取得成功は、それぞれ必要な材料だけを持つ。
- 各レポートがシナリオの比較参加可否と判定を所有し、`ScenarioVerdicts`と`ScenarioComparison`が比較の契約を共有する。同じ対象・同じユニットの、異なるバックエンドによる判定だけを比較する。判定集合の一実行予算は128バックエンド（最大8,128組）。入力の読取中に増大しても上限で停止し、同じスナップショットに意味の検査を適用する。不適合な比較入力は利用不能のレポートとして明示する。比較面は従来どおりシナリオだけとする。
...
## 型付きファーストクラスコレクションの操作契約（2026-09-06）

この過去の判断のうち、mapの汎用型返却は [ADR 0001: ドメイン固有のコレクション型を保つ](adr/0001-concrete-collection-mapping.ja.md) で置き換えた。

オーナーが指定したScala風の操作契約へ統一した。`NonEmptyFirstClassCollection<E>`は反復、`at`、`head`、`tail`、`include`、`exists`、`filter`、`map`を定義し、`FirstClassCollection<E>`が継承して`isEmpty`を加える。この継承は操作の共有であり、非空は具象型の構築契約で保証する。`FindingTargets.of(head, tail)`と固定の標準設置台帳は、従来の非空の生成規則を保つ。

要素とmapの変換先は`Equatable`を実装する。所属判定は、必要な所有者と内容を含むドメインの等価性へ委譲する。`exists`は最初の一致で短絡する。`tail`と`filter`は不変条件を満たせる範囲で具体型を保ち、非空集合を空にできる操作は空を許す集合を返す。`map`は変換先factoryを呼出側へ要求せず、`FirstClassCollection<U>`を返す。

既存112型は共通基底を通じて操作契約を実装し、汎用の変換結果には`ImmutableFirstClassCollection`を使う。属性カタログ、問いの判定、トレース状態、兄弟ユニット索引には、キーと所有者を保持するドメイン固有の項目型を定義した。旧iterableマーカーは削除した。トレース状態と問いの判定のfactoryは型付き項目を受け取り、以前のタプルやKeyedIndexの入力は削除した。

件数とスナップショットの検証はコンストラクタが所有する。共通上限は65,536要素とし、より厳しい既存予算は維持する。配列長と実際の反復を両方確認して保持値を作り、`add`も構築契約を迂回しない。想定内の入力不適合は型付き`parse`のResultで処理し、`of`のpanicは伝播する。要件IDの文書からの抽出はadapterへ集約し、識別子と集合のparseを通す。

入口での早期拒否により、解析不能なモデルを成功として報告する既存経路が表面化した。要件・設計の検証entry計4個で、`model-unreadable`は`pass: false`とする。利用不能の診断文書は保存する。ソルバ実行前に上限超過を拒否したモデルも、検査成功として報告しない。

## トレース状態の等価性と復号境界（2026-09-07）

トレース状態は属性パスと観測値の対応を表す。等価性はキーの有無と値自身の等価性で判定する。挿入順は反復と文書出力に反映するが、状態の同一性には含めない。属性の欠落と、明示的なnull値は区別する。トレースのステップ順は維持する。重複した属性パスは、最初の挿入位置を保って最後の値を採用する。

共通コレクション操作の導入時に加えた位置ごとの比較を、この規則へ変更する。すべての状態を整列して正準化する方法でも順序依存を除けるが、既存の出力順が変わるため採用しない。反復順と既存の文書出力順を保持したうえで、キーを使って対応する値を比較する。JSONオブジェクトの標準の列挙規則も維持し、整数添字のキーは数値昇順で出力する。ネストしたトレース値の等価性は既存の契約を維持する。

受理した属性キーは、JavaScriptオブジェクトのプロトタイプ上のプロパティ名と重なるものも含めて文書に保持する。文書構築ではキーをデータのプロパティとして扱い、プロトタイプの設定として解釈させない。

ITFの復号結果は、検証済みの`TraceStates`をResultに包んで返す。各状態の属性数と、トレース全体のステップ数を、それぞれのコレクションのparseで検証する。deadlock・不変条件違反・時相検証の経路は、その集合を直接受け取り、復号失敗は既存の利用不能の診断へ渡す。外部由来の配列をofで再構成したり、コンストラクタのpanicを捕捉したりしない。文書サイズとコレクション件数の上限値は維持する。

## コレクション契約の迂回を層で禁じる（2026-09-07）

ファーストクラスコレクションを配列や生の反復へ落として処理する経路を、型情報で検出するリンターを置いた。検出するのは4つの形である。コレクションに対する`for...of`、spread展開（`[...c]`）、`Array.from`、`toArray()`の呼出しである。式の型とその基底が共通契約の宣言へ辿り着くものだけをコレクションと見なすので、同名のメソッドを持つ配列や`Set`は拾わない。コレクション自身の実装（`extends`または`implements`で契約名を挙げるクラスの本体）は、反復と配列化がその型の責務そのものなので対象外にする。

適用範囲は層で切る。ドメイン層・ユースケース層・infrastructure層では迂回を許さない。インターフェイスアダプタ層と合成ルートは外界との境界であり、ドメインのモデルをSMTやQuintの本文へ翻訳することが責務そのものなので、反復も配列化も許す。テストとスクリプトはどの規則の対象にもしない。テストはコレクションの公開面が約束どおりかを確かめる場所であり、反復そのものを禁じると契約を検証できない。

迂回の解消は、契約にある操作へ寄せるか、その順序や射影に意味のある名前を付けてコレクション型自身の操作にするかのいずれかで行う。呼出側に比較関数を渡させる形は採らない。順序には名前を付ける。1周で2つ以上の値を蓄積するループや、例外を捕捉するループを`foldLeft`へ畳み込むと読みにくくなるので、そうした走査はコレクション型の名前つき操作の中へ移す。

走査そのものを契約の操作へ移すと意味か読みやすさが壊れる箇所は、免除表に理由と件数を書いて残す。免除は表にする——暗黙の除外や名前の部分一致による例外は作らない。件数を持たせるので、免除した場所に新しい迂回が増えれば落ちるし、迂回が消えて表だけ残れば陳腐化として落ちる。免除を増やすのも減らすのも表の更新を伴う。

kernelのコレクションには、呼出側に配列化を強いない口を足した。`FindingTargets.parseWithTail`は先頭を明示して続きを対象列そのものから受け、`ErrorMessages.asDiagnostics`は保持する診断を`collect`へそのまま流せる形で返す。`DeclaredBindings.matchesVerbatim`と`verbatimHashCode`は、値を`describe()`の文字列一致で見る逐語の関係に名前を与えた。これは要素自身の等価性（`Declaration`の`jsonEquals`、キー順非依存）より厳しい関係で、IR宣言の往復同一性を見る面が使う。

## バンドルサイズ上限の見直し（2026-09-07）

生成物1本あたりの上限を512 KiBから1 MiBへ引き上げた。上限の目的は異常な肥大化の検出であって特定の数値ではない。2026-09-03に512 KiBを置いた時点の実測は49〜300 KBだったが、その後のドメイン型の増加で設計系3本が533 KBに達し、上限を超えた。閾値を通すために単位や解釈を選ばず、上限自体を実測の約2倍へ置き直す。テスト内の実測値の記述も現状へ更新する。
