...
**規則**: adapter は既存の文書デコーダーで外部形式を解き、型付きの値から domain を構築する。復元にも `of` / `parse` と同じ不変条件が適用される。`decodeDomainValues` は復号中の `IllegalArgumentException` を失敗値へ変換し、実装上の例外は送出する。
...
**規則**: `of` の引数が値の不変条件に反する場合、コンストラクタは `IllegalArgumentException` を送出する。呼び出し側は通常の業務分岐として捕捉しない。入力不正を回復可能な結果として扱う `parse` と文書復号の境界だけが、この例外を `Result` に変換する。
