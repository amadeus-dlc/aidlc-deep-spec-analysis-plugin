...
| Refcheck sensors (design phase 1) | `sensors/aidlc-deep-spec-refcheck-{domain,contract,functional}.md` + `tools/aidlc-sensor-deep-spec-refcheck-*.ts` + `tools/deep-spec-lib.ts` | Solver-free, LLM-free reference/structure integrity for the design artifacts: the seven `components.md` well-formedness rules, `contract-summary.md` unit/spec-block/DAG-edge checks, and per-unit functional-design checks (entities types/ranges/relationships, BR rule ids + FR sources, state machine ↔ allowed values, drift vs the component catalogue). Contributed to the core design stages via `adds.sensors` + fix-or-record fragments; findings in `deep-spec-refcheck/*.json` (contract 2, `method: static`, self-validated). |
| Doctor | `tools/deep-spec-analysis-doctor.ts` | Advisory availability checks (z3-solver, node, quint, Apalache) with install commands, requirements-verification coverage (unverified/stale intents), and a report-only structural-debt scan of existing design artifacts. |
