...
| IR sensor | `sensors/aidlc-deep-spec-ir-valid.md` + `tools/aidlc-sensor-deep-spec-ir-valid.ts` + `tools/requirements/{domain,usecase,adapter}/` | Schema conformance + semantic checks + frRefs reverse-verified against requirements.md + `sourceDigest` recomputed and rejected on drift (the error carries the expected value). |
