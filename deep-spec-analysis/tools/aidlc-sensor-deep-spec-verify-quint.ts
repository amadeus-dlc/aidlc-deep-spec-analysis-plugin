...
import { existsSync as existsSync2, mkdirSync as mkdirSync3, readFileSync as readFileSync4, readdirSync, renameSync as renameSync3, rmSync as rmSync3 } from "fs";
...
import { existsSync as existsSync3, mkdtempSync, readFileSync as readFileSync5, readdirSync as readdirSync2, rmSync as rmSync4, writeFileSync as writeFileSync3 } from "fs";
...
`);
  ...
  const findingsSchemaFile = readContractSchema(join5(dirname3(fileURLToPath(import.meta.url)), "data", "deep-spec-findings-schema.json"));
  ...
  const useCase = new VerifyRequirementsQuintUseCase(new FormalModelRepositoryImpl, new VerificationDirectoryRepositoryImpl(new DirectoryFinalizationLock(new SystemClock, {
    self: () => process.pid,
    statusOf: (pid) => {
      try {
        process.kill(pid, 0);
  ...
  }, VERIFICATION_LOCK_BASENAME)), findingsSchema, new QuintClientImpl({
