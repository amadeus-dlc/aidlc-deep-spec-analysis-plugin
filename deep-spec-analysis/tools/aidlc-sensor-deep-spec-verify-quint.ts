...
import { existsSync as existsSync2, mkdirSync as mkdirSync2, readFileSync as readFileSync3, readdirSync, writeFileSync as writeFileSync2 } from "fs";
...
class VerificationReportRepositoryImpl {
  ...
  findAllByDirectory(directory) {
    ...
    for (const file of entries) {
      try {
        const raw = JSON.parse(readFileSync3(join2(directory.asString(), file), "utf-8"));
...
import { existsSync as existsSync3, mkdtempSync, readFileSync as readFileSync4, readdirSync as readdirSync2, rmSync as rmSync2, writeFileSync as writeFileSync3 } from "fs";
...
`);
  ...
  const useCase = new VerifyRequirementsQuintUseCase(new FormalModelRepositoryImpl, new VerificationReportRepositoryImpl(join4(dirname3(fileURLToPath(import.meta.url)), "data", "deep-spec-findings-schema.json")), new QuintClientImpl({
