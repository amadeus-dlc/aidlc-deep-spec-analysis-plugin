...
`);
  ...
  const findingsSchema = readFindingsSchema(join5(dirname3(fileURLToPath(import.meta.url)), "data", "deep-spec-findings-schema.json"));
  const useCase = new VerifyRequirementsQuintUseCase(new FormalModelRepositoryImplementation, new VerificationDirectoryRepositoryImplementation(new DirectoryFinalizationLock(new SystemClock, {
    self: () => process.pid,
    statusOf: (pid) => {
      try {
        process.kill(pid, 0);
  ...
  }, VERIFICATION_LOCK_BASENAME)), findingsSchema, new QuintClientImplementation({
