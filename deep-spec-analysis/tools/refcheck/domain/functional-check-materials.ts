...
function runFunctionalChecksImpl(materials: {
  ...
  const entities: DeclaredEntities | null = materials.entities.match({
    ...
    wrongFenceCount: (found) => {
      report.finding(FD_E1, "structure-invalid", [FD_E1.asCheckTarget()], [ref(entitiesArt, "yaml fence")],
    ...
    unparseable: (line, error) => {
      report.finding(FD_E1, "structure-invalid", [FD_E1.asCheckTarget()], [ref(entitiesArt, `yaml fence (line ${line.asNumber()})`)],
    ...
    extracted: (model) => {
      for (const e of model.shapeErrors()) {
        report.finding(FD_E1, "structure-invalid", [FD_E1.asCheckTarget()], [ref(entitiesArt, e.element().asString())], e.detail());
      }
      for (const dup of model.entities().duplicatesByName()) {
        report.finding(FD_E1, "structure-invalid", [TargetIds.safe("entity", dup.name().asString())], [ref(entitiesArt, `${dup.element().asString()}.name`, dup.name().asString())],
          `entity "${dup.name().asString()}" is declared more than once`);
      }
      for (const e of model.entities()) {
        for (const dup of e.attrs().duplicatesByName()) {
          report.finding(FD_E1, "structure-invalid", [TargetIds.safe("attr", `${e.name().asString()}.${dup.name().asString()}`)], [ref(entitiesArt, `${dup.element().asString()}.name`, dup.name().asString())],
    ...
    for (const e of entities.entities()) {
      for (const a of e.attrs()) {
        ...
        if (a.declaresAllowedValuesOnNonEnumerableType()) {
          report.finding(FD_E2, "structure-invalid", [attrId], [ref(entitiesArt, a.element().asString(), a.typeToken())],
        ...
        if (a.boundsInverted()) {
          report.finding(FD_E3, "structure-invalid", [attrId], [ref(entitiesArt, a.element().asString(), `min ${a.min()?.asNumber()} > max ${a.max()?.asNumber()}`)],
            `"${label}": min ${a.min()?.asNumber()} exceeds max ${a.max()?.asNumber()}`);
        }
        if (a.defaultBelowMin()) {
          report.finding(FD_E3, "structure-invalid", [attrId], [ref(entitiesArt, a.element().asString(), a.def()?.render() ?? "")],
        ...
        if (reference !== null && !entities.entities().resolvesReference(reference)) {
          report.finding(FD_E6, "reference-broken", [attrId], [ref(entitiesArt, a.element().asString(), reference.asString())],
    ...
    for (const r of entities.allRels()) {
      for (const endpoint of [r.from(), r.to()]) {
        if (endpoint !== null && !entities.entities().containsNamed(endpoint.asString())) {
          report.finding(FD_E4, "reference-broken", [TargetIds.safe("entity", endpoint.asString())], [ref(entitiesArt, r.element().asString(), endpoint.asString())],
            `relationship endpoint "${endpoint.asString()}" is not a declared entity`);
        }
      }
      if (r.cardinalityOutsideClosedSet()) {
        report.finding(FD_E5, "structure-invalid", [FD_E5.asCheckTarget()], [ref(entitiesArt, r.element().asString(), r.cardinality()?.asString() ?? "")],
          `cardinality "${r.cardinality()?.asString()}" is not in the closed set 1:1 | 1:N | N:1 | N:M`);
      }
      if (r.cardinalityWithoutDirection()) {
        report.finding(FD_E5, "structure-invalid", [FD_E5.asCheckTarget()], [ref(entitiesArt, r.element().asString())],
  ...
  const rules: RuleDecls | null = materials.rules.match({
    ...
    wrongFenceCount: (found) => {
      report.finding(FD_R1, "structure-invalid", [FD_R1.asCheckTarget()], [ref(rulesArt, "yaml fence")],
    ...
    unparseable: (line, error) => {
      report.finding(FD_R1, "structure-invalid", [FD_R1.asCheckTarget()], [ref(rulesArt, `yaml fence (line ${line.asNumber()})`)],
        `yaml block does not parse in the supported subset: ${error}`);
      blockRs("blocked by FD-R1: the rules yaml block is unusable");
      return null;
    },
    noRulesList: () => {
      report.finding(FD_R1, "structure-invalid", [FD_R1.asCheckTarget()], [ref(rulesArt, "rules")],
  ...
  if (rules !== null) {
    ...
    for (const r of rules) {
      ...
      if (id === null) continue;
      if (!id.matchesShape()) {
        report.finding(FD_R2, "structure-invalid", [FD_R2.asCheckTarget()], [ref(rulesArt, `${r.element().asString()}.id`, id.asString())],
          `rule id "${id.asString()}" does not match BR{group}.{seq}`);
        continue;
      }
      if (seenIds.has(id.asString())) {
        report.finding(FD_R2, "structure-invalid", [id.asString()], [ref(rulesArt, `${r.element().asString()}.id`, id.asString())],
    ...
    present: (machines) => {
      ...
      for (const m of machines) {
        ...
        if (m.unsupported() !== null) {
          report.skip(FD_S1, "unrecognized-format", `${el}: ${m.unsupported()}`);
          report.skip(FD_S2, "unrecognized-format", `${el}: ${m.unsupported()}`);
          continue;
        }
        const ent = entities.entities().byNormalizedName(entity.normalized());
        if (!ent) {
          report.finding(FD_S1, "consistency-mismatch", [TargetIds.safe("entity", entName)], [ref(specArt, el, entName)],
...
export class FunctionalCheckMaterials {
  ...
  runChecks(report: ReferenceCheckReport): void {
    runFunctionalChecksImpl(this.#seed, report);
  }
}
