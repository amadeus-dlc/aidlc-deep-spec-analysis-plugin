...
`);
  ...
  const useCase = new VerifyDesignSmtUseCase(new DesignModelRepositoryImpl, new DesignReportRepositoryImpl(join7(toolsDir, "data", "deep-spec-findings-schema.json")), new SiblingBackendClientImpl({
    siblingToolPaths: {
      smt: join7(toolsDir, "aidlc-sensor-deep-spec-verify-smt.ts"),
      quint: join7(toolsDir, "aidlc-sensor-deep-spec-verify-quint.ts")
    },
    workingDirectory: process.cwd()
  }), new RefinementMaterialsRepositoryImpl(join7(toolsDir, "data", "deep-spec-refinement-map-schema.json")), new RefinementSolverClientImpl({
    childHostPath: join7(toolsDir, "aidlc-sensor-deep-spec-verify-smt.ts"),
