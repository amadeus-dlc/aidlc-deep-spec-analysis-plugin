...
export class DoctorPresenter {
  ...
  functionalCoverage(coverage: UnitCoverage): Check[] {
    ...
    const rows: Check[] = coverage.refinementStale().map((row) => ({
      pass: false,
      label: `deep-spec-analysis: intent ${row.space}/${row.intent} re-verified its requirements after the last design verification (refinement evidence is stale)`,
      fix:
        `Make it the active intent (\`bun ${this.#harnessDir}/tools/aidlc-utility.ts intent ${row.intent}\`), ` +
