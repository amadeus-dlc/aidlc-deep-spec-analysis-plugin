...
export class RefinementStatus {
  ...
  skipFor(target: TargetId, unit: string): DesignSkipped | null {
    if (this.#kind === "waived") return DesignSkipped.reconstitute({ target, reason: "waived", unit, detail: this.#text });
    if (this.#kind === "capability") return DesignSkipped.reconstitute({ target, reason: "capability", unit, detail: this.#text });
    return null;
  }
}
