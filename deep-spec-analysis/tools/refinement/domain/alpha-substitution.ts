...
// 旧 refinement-lib の primeAll / alphaExpr / alphaEquality からの逐語移植
// （AttributeMapping の閉じたユニオン化に伴う分岐の読み替えのみ）。

import { idCompare, sortedUnique } from "../../kernel/domain/index.ts";
import type { Expression } from "../../kernel/domain/index.ts";
import type { AttributeMapping } from "./refinement-map.ts";
import type { RefinementAttribute } from "./refinement-requirements.ts";
...
export interface AlphaContext {
  byReq: Map<string, AttributeMapping>;
  reqAttrByPath: Map<string, RefinementAttribute>;
}
...
export function alphaExpr(ctx: AlphaContext, e: Expression, post: boolean): Expression {
  if (e.op === "eq" || e.op === "ne") {
    const [a, b] = e.args ?? [];
...
export function alphaEquality(ctx: AlphaContext, reqPath: string): Expression | null {
