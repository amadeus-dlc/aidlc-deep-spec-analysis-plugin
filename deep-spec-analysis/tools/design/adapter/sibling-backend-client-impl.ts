...
// 持つ（形式知識）。toolsDir・作業ディレクトリは entry が注入する
...
import type { ReachabilityProbe, SiblingBackendClient, SiblingLoweredRun } from "../usecase/index.ts";
...
export class SiblingBackendClientImpl implements SiblingBackendClient {
  ...
  #spawn(backend: "smt" | "quint", loweredDoc: Json, wallTimeoutMs: number): { exit: number | null; doc: Json | null; note: string } {
    const tool = join(this.#config.toolsDirectory, `aidlc-sensor-deep-spec-verify-${backend}.ts`);
