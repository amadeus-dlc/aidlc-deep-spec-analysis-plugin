...
// findings document" 経路）、SiblingVerdictDoc の "unreadable" は読めたが
// 文書として成立しないことを区別する（旧挙動の凍結）。

import type { DesignUnit, LoweredUnit, SiblingVerdictDoc } from "../domain/index.ts";

export interface SiblingLoweredRun {
  exit: number | null;
  doc: SiblingVerdictDoc | null;
  note: string;
}
