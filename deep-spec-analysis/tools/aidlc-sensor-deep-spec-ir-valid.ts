...
function findRequirementsFile(recordDir) {
  ...
  if (directStat.error.kind === "io-failed")
    return { kind: "unreadable", cause: directStat.error.cause };
  ...
  for (const phase of [...phases.value].filter((entry) => entry.isDirectory()).map((entry) => entry.name).sort()) {
    ...
    if (candidateStat.error.kind === "io-failed")
      return { kind: "unreadable", cause: candidateStat.error.cause };
  }
  return { kind: "absent" };
}
