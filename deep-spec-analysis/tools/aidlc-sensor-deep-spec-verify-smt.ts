...
class VerificationDirectoryRepositoryImplementation {
  ...
  #publish(aggregate, candidate, directory) {
    const directoryPath = directory.asString();
    const backendPath = join4(directoryPath, candidate.id().fileName());
    const crossPath = join4(directoryPath, CROSS_CHECK_BASENAME);
    const stalePath = join4(directoryPath, STALE_CROSS_CHECK_BASENAME);
...
async function solveSmtChild() {
  let payload;
  try {
    payload = JSON.parse(readFileSync7(0, "utf-8"));
  } catch (err2) {
