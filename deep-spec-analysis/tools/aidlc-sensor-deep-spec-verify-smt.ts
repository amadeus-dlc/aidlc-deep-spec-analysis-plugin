...
function parseFormalModel(raw) {
  ...
  for (const bg of Array.isArray(raw.background) ? raw.background : []) {
    ...
    const constructed = BackgroundAssumption.parse({ id: id.value, assert: bg.assert });
    if (!constructed.ok)
      return err(JSON.stringify(constructed.error));
...
`);
  ...
  return {
    ...
    plan: SatisfiabilityModuloTheoriesVerificationPlan.of({
      ...
      gapTriggers: KeyedIndex.of([...gapTriggers].map(([trigger, ids]) => [TriggerName.of(trigger), TargetIdentifiers.of(Array.from(ids, (raw) => TargetIdentifier.of(raw)))])),
...
`);
  ...
  const findingsSchema = readFindingsSchema(join5(dirname3(selfPath), "data", "deep-spec-findings-schema.json"));
  const useCase = new VerifyRequirementsSatisfiabilityModuloTheoriesUseCase(new FormalModelRepositoryImplementation, new VerificationDirectoryRepositoryImplementation(new DirectoryFinalizationLock(new SystemClock, {
    self: () => process.pid,
    statusOf: (pid) => {
      try {
        process.kill(pid, 0);
  ...
  }, VERIFICATION_LOCK_BASENAME)), findingsSchema, new Z3SolverClientImplementation({
    selfPath,
