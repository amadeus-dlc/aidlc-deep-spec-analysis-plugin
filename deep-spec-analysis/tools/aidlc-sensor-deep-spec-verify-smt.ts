...
function parseFormalModel(raw) {
  const decoded = decodeDomainValues(() => parseFormalModelValue(raw));
  return decoded.ok ? decoded.value : decoded.error;
}
function parseFormalModelValue(raw) {
  if (!isObject(raw))
  ...
  for (const ent of Array.isArray(schema.entities) ? schema.entities : []) {
    ...
    for (const attr of Array.isArray(ent.attributes) ? ent.attributes : []) {
      ...
      attributes.push(AttributeDeclaration.of({
        ...
        kind,
        min: typeof t.min === "number" ? AttributeBound.of(t.min) : undefined,
        max: typeof t.max === "number" ? AttributeBound.of(t.max) : undefined,
  ...
  for (const bg of Array.isArray(raw.background) ? raw.background : []) {
    if (!isObject(bg) || typeof bg.id !== "string" || !isObject(bg.assert))
      continue;
    background.push(BackgroundAssumption.of({ id: BackgroundAssumptionId.of(bg.id), assert: bg.assert }));
  }
  return {
...
`);
  ...
  return {
    ...
    plan: SmtVerificationPlan.of({
      ...
      gapTriggers: KeyedIndex.of([...gapTriggers].map(([trigger, ids]) => [TriggerName.of(trigger), TargetIds.of(Array.from(ids, (raw) => TargetId.of(raw)))])),
      scenarioQueries: KeyedIndex.of([...scenarioQueries].map(([sc, qid]) => [ScenarioId.of(sc), QueryLabel.of(qid)]))
    })
  };
}
...
async function solveSmtChild() {
  let payload;
  try {
    payload = JSON.parse(readFileSync4(0, "utf-8"));
  } catch (err2) {
...
class VerificationDirectoryRepositoryImpl {
  ...
  #siblingsOf(directory) {
    if (!existsSync2(directory.asString()))
      return ok([]);
    let entries;
    try {
...
import { existsSync as existsSync3, mkdtempSync, readFileSync as readFileSync6, readdirSync as readdirSync2, rmSync as rmSync4, writeFileSync as writeFileSync3 } from "fs";
...
class QuintClientImpl {
  ...
  check(model) {
    ...
    try {
      ...
      const runs = QuintRuns.of({
        machine: machineRun,
        temporals: KeyedIndex.of([...temporals].map(([id, v]) => [ObligationId.of(id), v])),
        scenarios: KeyedIndex.of([...scenarios].map(([id, v]) => [ScenarioId.of(id), v]))
      });
  ...
  #runQuint(args, itfPath, timeoutMs, cwd) {
    ...
    if (itfPath && existsSync3(itfPath)) {
      try {
        itf = readFileSync6(itfPath, "utf-8");
      } catch {
        itf = null;
      }
    }
...
function buildView(ir) {
  ...
  for (const ob of Array.isArray(ir.obligations) ? ir.obligations : []) {
    ...
    obligations.push(IrObligationDecl.of({
      ...
      temporal: temporal === null ? undefined : IrTemporalDecl.of({
...
class IrValidationMaterialsRepositoryImpl {
  ...
  #findById(id) {
    ...
    let bytes;
    try {
      bytes = readFileSync7(outputPath);
    } catch (e) {
...
function parentMain() {
  ...
  const reportLocation = ArtifactPath.parse(join5(dirname3(flags.outputPath), VERIFY_DIRNAME));
...
`);
  ...
  const findingsSchemaFile = readContractSchema(join5(dirname3(selfPath), "data", "deep-spec-findings-schema.json"));
  ...
  const useCase = new VerifyRequirementsSmtUseCase(new FormalModelRepositoryImpl, new VerificationDirectoryRepositoryImpl(new DirectoryFinalizationLock(new SystemClock, {
    self: () => process.pid,
    statusOf: (pid) => {
      try {
        process.kill(pid, 0);
  ...
  }, VERIFICATION_LOCK_BASENAME)), findingsSchema, new Z3SolverClientImpl({
    selfPath,
