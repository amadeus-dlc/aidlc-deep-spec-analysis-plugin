// Repository 共有のエラー語彙 — アウトプットポートの一部（use-case 層）。
// ポートごとに固有エラー型を乱造しない。ドメイン層には置かない：Repository
// をドメインから遠ざけ、ドメインオブジェクトが内部で Repository に触れる
// 構造を作らないため（オーナー裁定 2026-08-30）。
