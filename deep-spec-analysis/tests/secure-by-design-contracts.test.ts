...
import {
  ...
  FindingKind, IrVersion, RequirementId, SkipReason, TargetId, VerificationMethod,
} from "@deep-spec/kernel-domain";
import {
  BackgroundAssumptionId, ObligationId, ScenarioId, SourceAnchor,
} from "@deep-spec/requirements-domain";
import {
  BrRef, DesignBackgroundId, DesignMachineId, DesignObligationId, DesignScenarioId, DesignTransitionId,
} from "@deep-spec/design-domain";
import { BlockIndex, BusinessRuleId, CardinalityNotation, ContractParty, FenceCount, LineNumber, NumericBound } from "@deep-spec/refcheck-domain";
...
describe("サイズ: 字句・構文の検査に先行する", () => {
  test("長大かつ字句不正のIDは、まずサイズ違反になる", () => {
    ...
    rejects(RequirementId, raw);
    expect(RequirementId.parse(raw)).toEqual({ ok: false, error: { kind: "requirement-id-too-long", raw: 129 } });
  });
...
const numberedIds = [
  ["ObligationId", ObligationId, "OB"], ["ScenarioId", ScenarioId, "SC"],
  ["BackgroundAssumptionId", BackgroundAssumptionId, "BG"],
  ["DesignObligationId", DesignObligationId, "DOB"], ["DesignScenarioId", DesignScenarioId, "DSC"],
  ["DesignBackgroundId", DesignBackgroundId, "DBG"], ["DesignMachineId", DesignMachineId, "SM"],
  ["DesignTransitionId", DesignTransitionId, "TR"],
] as const;
...
describe("字句的内容: 許容文字とエンコーディング", () => {
  ...
  test("要件・規則IDとバージョンも非ASCII数字を数字として扱わない", () => {
    rejects(RequirementId, "FR-１");
    rejects(BrRef, "BR1.１");
    rejects(BusinessRuleId, "BR１.1");
    rejects(IrVersion, "1.０.0");
...
describe("構文: 正当な文字だけでも組み合わせが不正なら拒否する", () => {
  ...
  test("要件参照と業務規則参照の構造を区別する", () => {
    for (const raw of ["FR--1", "FR-1..2", "FR-.1", "FR-1."]) rejects(RequirementId, raw);
    for (const factory of [BrRef, BusinessRuleId]) for (const raw of ["BR-1.1", "BR1", "BR1..1", "BR1.1.1"]) rejects(factory, raw);
    rejects(TargetId, "component:");
    rejects(TargetId, "OB_1");
  });
  test("バージョンの構文契約と正規化の有無を区別する", () => {
    for (const raw of ["1.2", "1.2.3.4", "1..3", ".1.2"]) {
      rejects(IrVersion, raw);
      rejects(PluginVersion, raw);
    }
    rejects(PluginVersion, "01.2.3");
    rejects(PluginVersion, "1.2.3-beta");
    expect(IrVersion.parse("01.2.3").ok).toBe(true);
...
describe("意味: 値域・所属・値同士の関係", () => {
  ...
  test("診断用の宣言値は、不適合な記述を検証済みの値に昇格させない", () => {
    const declaration = DeclaredBindingValue.parse(1.5);
    expect(declaration.ok).toBe(true);
    if (declaration.ok) {
      expect(declaration.value.fits(AttributeKind.of("int"), () => false)).toBe(false);
      expect(BindingValue.resolve(declaration.value).ok).toBe(false);
    }
    const member = DeclaredBindingValue.parse("closed");
    expect(member.ok).toBe(true);
    if (member.ok) {
      expect(member.value.fits(AttributeKind.of("enum"), (value) => value === "open")).toBe(false);
      expect(member.value.fits(AttributeKind.of("enum"), (value) => value === "closed")).toBe(true);
    }
  });
