...
| 監査項目 | 修正 | 回帰検証 |
|---|---|---|
| 1. timeoutを非到達へ変換 | `SiblingVerdictDocument.reachabilityOf` が到達の証跡・bounded探索の完了・未検証を区別する。adapterは復号とportへの写像を行う | timeout／compile-error／capability／unavailable、証跡不足、simulationを非到達にしない。実adapterを経由したusecaseでも誤ったfindingが出ない |
...
- `bun test --coverage`: 606成功、1スキップ、0失敗。34ファイル、607テスト、3386 assertions。
- カバレッジ表示: 関数99.79%、行99.94%。計測対象は `bunfig.toml` に定義された範囲。
...
相対ゲートは既存の `scripts/coverage.ts` の `runGate` を `baseRef: "HEAD"` で実行した。
