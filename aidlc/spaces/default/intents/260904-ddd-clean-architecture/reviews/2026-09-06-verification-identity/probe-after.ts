import { requireSuccess } from "../../../../../../../deep-spec-analysis/tests/result-fixtures.ts";
...
const source = "90874c14037b73824372d2ca2ebb6bfe9beabb22";
...
const both = coverage([]);
assert.notDeepEqual(u1Only, u2Only);
assert.notDeepEqual(u1Only, both);
evidence.crossCheckedScopeLoss = { compared: ["u1のみ", "u2のみ", "両方"],
  documentsIdentical: false, u1Only, u2Only, both };
...
assert.deepEqual(unknown.remappedTargets, []);
...
const extended = requireSuccess(restored.extendedWith(D.RefinementQuintInvariants.of([
...
assert.deepEqual(duplicateIds, ["OB-2", "OB-1"]);
evidence.loweredIdentityCollision = { before: requireSuccess(restored.index().resolveDesignTarget(D.LoweredIdentifier.of("OB-2"))).design.asString(),
  after: requireSuccess(extended.index().resolveDesignTarget(D.LoweredIdentifier.of("OB-2"))).design.asString(), duplicateIds };
...
const composition = [9998, 10000].map(inputNodes => {
  ...
  assert.equal(requireSuccess(largeUnit.lowered({ synthetics: false })).obligations().count(), 1);
  const outcome = largeUnit.lowered({ synthetics: true });
  const failure = outcome.ok ? null : outcome.error;
...
assert.equal(composition[1].failure?.kind, "expression-too-large");
...
const transitionComposition = [9996, 10000].map(inputNodes => {
  ...
  const outcome = [...input][0].lowered({ synthetics: false });
  const failure = outcome.ok ? null : outcome.error;
...
assert.equal(transitionComposition[1].failure?.kind, "expression-too-large");
...
assert.equal(foreignComparison.ok, false);
...
evidence.comparisonModelScope = { sameHash: false, comparisonAccepted: foreignComparison.ok,
  comparisonDisagrees: null, normalReportPathFindings: safeReportPath.findingsCount(), normalReportPathComparisons: safeReportPath.crossChecked()?.toArray().length };
...
let foreignFailure: { name: string; message: string } | null = null;
try { materials.prepare(otherRecord); }
catch (error) {
  if (!(error instanceof Error)) throw error;
  foreignFailure = { name: error.constructor.name, message: error.message };
}
assert.equal([...correct].length, 1);
assert.equal(foreignFailure?.name, "IllegalArgumentException");
evidence.materialModelScope = { materialsFor: materials.id().modelArtifactPath().asString(), requestedModel: otherRecord.id().artifactPath().asString(),
  sameModelId: false, sameContentHash: true, plansForCorrectModel: [...correct].length, foreignFailure };

writeFileSync(join(import.meta.dir, "runtime-after.json"), JSON.stringify(evidence, null, 2) + "\n");
