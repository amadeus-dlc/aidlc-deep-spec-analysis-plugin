...
`deep-spec-analysis/` の外部仕様と形式検証結果を変えずに、Design／Refinement 検証経路の責務境界、不変条件、永続化整合性、重複実装を整理する。あわせて、この変更の設計検証中に実測した AI-DLC のゼロ Unit 経路の不整合を、同じ Intent の作業として修正する。狙いは一般論としてのレイヤ名合わせではなく、失敗を成功に見せないこと、不正な値を正常生成できないこと、同じ変更理由を一つの所有者へ寄せること、および Unit を生成しない構成でも検証手続きが一貫して完走することである。[desc] [Q1] [Q4] [Q5]
...
- **FR2.5** lock 取得は待機や再試行を行わない単発の排他取得とし、競合時は直ちに既存の失敗チャネルへ返さなければならない。writer終了時は成功・失敗の双方で解放し、異常終了で残ったlockは所有者識別子とlease期限により次のwriterが回復可能でなければならない。[Q3]
- **FR2.6** lock取得失敗、競合、stale lock回復、回復失敗は区別可能な内部結果として記録し、外部向け文言を変更せずに診断できなければならない。[Q3] [Q4]
...
- Given live writer が同じdirectoryのlockを保持する、When別writerが取得を試みる、Then待機・再試行せず失敗を返し、独立directoryのwriterは影響を受けない。
- Given writer がlockを残して異常終了する、When lease期限後に次のwriterが所有者情報を検証する、Then stale lockを置換して処理できる。lease期限前または所有者がliveなら置換してはならない。
...
### FR4. Refinement の Design 内への統合

- **FR4.1** 現在の `@deep-spec/refinement-domain` の実装を Design context 内へ統合し、独立workspace packageとしてのRefinementを廃止しなければならない。[Q2]
...
- **FR4.5** `design/domain/refinement/` のような新しい局所階層は作らず、既存 `design/domain` の分類規則へflatに統合しなければならない。[feedback]
...
- **FR6.3** 新しいdomain service、自由関数、`*Plan`型が必要になった場合は、実測した問題と代替案を示して実装前に個別の人間裁定を得なければならない。[Q4] [RE:structure]
...
- **FR7.3** 既存の検査規則を削って変更を通してはならない。[RE:structure]
...
- **FR8.1** 解決された Unit 集合がゼロ件の構成では、Units Generation が実行済みか `SKIP` かにかかわらず、Unit 単位として定義された stage の質問確認、review confirmation、traceability が、同じ stage-level artifact directory を一貫して解決しなければならない。[Q5]
- **FR8.2** stage-level 解決は少なくとも `functional-design`、`nfr-requirements`、`nfr-design`、`infrastructure-design`、`code-generation` の既存 zero-Unit 対象へ同じ規則で適用し、特定 stage だけの例外実装にしてはならない。[Q5]
- **FR8.3** 修正の正本は `aidlc-workflows/core/` とし、package generator により全 harness の配布物へ反映する。現在の Intent を継続するため、workspace 直下の `.codex/tools/` にある導入コピーにも同じ修正を適用しなければならない。[Q5]
- **FR8.4** 修正前には、stage-level `functional-design` の traceability が Unit 導出で失敗する例と、stage-level questions が review confirmation に発見されない例を再現する回帰テストを追加し、修正後に両方を通さなければならない。[Q5]
- **FR8.5** `aidlc-workflows/AGENTS.md` の公開変更規則に従い、version、README badge、CHANGELOGを同じリリース番号へ更新しなければならない。[Q5]

**Acceptance criteria**

- Given 解決された Unit 集合がゼロ件で Functional Design が stage-level directory を使用する、When Units Generation が実行済みまたは `SKIP` の各状態でtraceability sensorとreview requestを実行する、Then Unit名を要求せず、実測したstage-level questions／review confirmation／traceabilityの失敗を再発させず、正しいrequirements／rules／questionsを解決する。
- Given canonical core の修正、When package generation と drift check を実行する、Then全harness配布物はcanonical sourceと一致し、生成差分が残らない。
- Givenゼロ Unit回帰テスト、When修正前後を比較する、Then修正前は意図した失敗を再現し、修正後は既存のper-Unit経路を壊さずgreenになる。
...
lock競合は待機・再試行なしで決定的に失敗させ、同じdirectoryだけを直列化する。異なるdirectory間は相互に遮断してはならない。stale lockの判定は注入可能な時計と所有者識別子で再現可能にし、取得、競合、回復、回復失敗をテストで観測できなければならない。[Q3]

### NFR3. Testability

変更前に全テストとtypecheckの基線を記録し、変更後も全既存テストをgreenに保つ。少なくともschema二重観測、cross-check読込／保存失敗、同一directoryの並行writer、strict creation拒否、tolerant hydration、Refinement統合後のpackage edge、zero-Unit traceability、zero-Unit review confirmationを再現する回帰テストを追加する。[RE:architecture] [RE:structure] [Q5]
...
- プラグインの改変対象は `deep-spec-analysis/` とする。加えて、同じ Intent に含めるzero-Unit修正に限り、`aidlc-workflows/core/`、対応する回帰テスト、generatorが生成する全harness配布物、公開version／README badge／CHANGELOG、および現在のworkspaceの `.codex/tools/` 導入コピーを改変対象とする。その他の `.claude/`、`sandbox/`、`node_modules/` は変更しない。
...
- zero-Unit経路の解決と必要なリリース情報更新を除く、installer、doctor、release、CIの機能変更。
...
- [Q5] `requirements-analysis-questions.md` Q5: A. ゼロ Unit 経路の不整合修正を同じ Intent に含める
- [feedback] ユーザー指示: `domain` 配下にRefinement専用directoryを作る部分最適を避け、既存の統一ルールへ合わせる
...
|---|---|---|---|---|---|
| R-01 | Critical | aidlc/spaces/default/intents/260904-ddd-clean-architecture/inception/requirements-analysis/requirements.md > FR8.1、FR8 Acceptance criteria | 発火条件は「解決された Unit 集合がゼロ件」に修正され、Units Generation が実行済み・`SKIP` の双方を明記した。受入条件も stage-level questions、review confirmation、traceability の実測失敗と既存 per-Unit 経路の非退行を検証するため、確認済み Q5 の範囲を満たす。 | 対応不要。 | Resolved |
| R-02 | Major | aidlc/spaces/default/intents/260904-ddd-clean-architecture/inception/requirements-analysis/requirements.md > FR2.5、FR2.6、NFR2 | lock は待機・再試行なしの決定的な競合失敗、成功・失敗時の解放、所有者と lease による stale lock 回復、独立 directory の非干渉、取得・競合・回復・回復失敗の観測可能性まで合否基準が追加された。性能・スケーラビリティ・信頼性・可観測性を QA が検証できる。 | 対応不要。 | Resolved |
| R-03 | Major | aidlc/spaces/default/intents/260904-ddd-clean-architecture/inception/requirements-analysis/requirements.md > FR4.5、Sources `[feedback]` | flat 配置は `[feedback]` にユーザー指示として明示され、Q2 の Design subdomain 統合を具体化する確認済み制約として追跡できる。誤っていた Q5 参照も除去された。 | 対応不要。 | Resolved |
| R-04 | Minor | aidlc/spaces/default/intents/260904-ddd-clean-architecture/inception/requirements-analysis/requirements.md > FR6.3、FR7.3、Sources | レビュー対象外だった `[memory]` 参照は削除され、domain service の裁定条件と構造ゲート維持はいずれもレビュー済みの `[RE:structure]` へ付け替えられた。 | 対応不要。 | Resolved |

### Summary

前回4件はすべて、確認済みQ&A、ユーザーの改訂指示、またはレビュー対象の上流契約へ追跡できる形で解消された。要件は実装範囲、互換性境界、失敗条件、ゼロ Unit の同一 Intent 内スコープを十分に明確化しており、承認可能である。
